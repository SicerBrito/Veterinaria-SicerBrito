...
namespace API.Controllers;
    ...
    public class ProveedorController : BaseApiController{
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<Pager<ProveedorDto>>> Get11([FromQuery] Params recordParams)
        {
            var record = await _UnitOfWork.Proveedores!.GetAllAsync(recordParams.PageIndex,recordParams.PageSize,recordParams.Search);
            var lstrecordsDto = _Mapper.Map<List<ProveedorDto>>(record.registros);
            return new Pager<ProveedorDto>(lstrecordsDto,record.totalRegistros,recordParams.PageIndex,recordParams.PageSize,recordParams.Search);
        }
